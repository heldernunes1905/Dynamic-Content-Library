...
document.getElementById("full").className = "active";
	...
	<?php
		foreach($contents as $content){
			...
			echo "Character Id: $content->character_id".'<br />';
      ...
      foreach($chars as $characters){
        echo "Played by: ";
        ?>
        <a href="<?= base_url() ?>index.php/staff/<?= $characters[0]->staff_id ?>"><?php echo $characters[0]->first_name ." ". $characters[0]->last_name.'<br />'?></a>
      <?php }
     }?>
...
function confirmRemoveComment(character_id,user_id) {
    var response = confirm("Do you want to remove content:");
    if (response) {
      window.location="<?= base_url() ?>index.php/removecharactercontent/<?= $user_id?>/" + character_id
    }
}
</script>
<?php
